...
function run(str, data) {
  const hexo = this;
  const options = hexo.config.descco_pipeline.html_minifier;
   ...
   */
  if (!enable) {
    log.info('Filter(descco_pipeline.html_minifier) is not enabled.Skipping it.');
    return Promise.resolve('Skipping descco_pipeline.html_minifier.');
  }
