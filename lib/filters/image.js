...
function run(str, data) {
  // Init.
  const hexo = this;
  const options = hexo.config.descco_pipeline.imagemin;
   ...
   */
  if (!enable) {
    log.info('Filter(descco_pipeline.imagemin) is not enabled.Skipping it.');
    return Promise.resolve('Skipping descco_pipeline.imagemin.');
  }
